...
function UploadPage({ showAlert }) {
  ...
  const handleSubmit = (data) => {
    setLoading(true);
    const promise = id ? api.updateProduct(id, data) : api.createProduct(data);
    promise
      .then(() => {
        showAlert(id ? 'Product updated successfully' : 'Product created successfully');
        navigate('/products');
      })
      .catch(() => showAlert('Failed to save product', 'error'))
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-800 dark:text-white">{id ? 'Edit Product' : 'Upload New Clothes'}</h2>
        <p className="text-gray-500 dark:text-gray-400">{id ? 'Update product information' : 'Add a new product to your clothing store'}</p>
      </div>
